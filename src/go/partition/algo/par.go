...
func sum_par(arr []int, size int, index int64) int {
	var sum int = 0
	...
	for w := 0; w < threads; w++ {
		...
		go func(w int) {
			...
			for i := 0; i <= thread_problems; i++ {
				...
				var problem_sum int = sum_par(arr[problem], len(arr[problem]), all)
				if problem_sum%2 != 0 {
					continue
				}
				...
				for j := int64(0); j < combs; j++ {
					var sum int = sum_par(arr[problem], len(arr[problem]), j)
					if sum == half_sum {
						result[problem] = 1
						break
					}
