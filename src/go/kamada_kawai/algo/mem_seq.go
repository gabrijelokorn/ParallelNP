...
func (kk *KamadaKawai) Mem_seq() {
	var delta_max_index int64 = kk.get_deltas_mem_seq()
	...
	for {
		...
		for {
			...
			d_x_m, d_y_m, d_xx_m, d_yy_m, d_xy_m := kk.get_derivatives_mem_seq(delta_max_index)
			...
			kk.Deltas[delta_max_index] = kk.get_delta_m_mem_seq(delta_max_index)
		...
		var temp int64 = kk.update_deltas_mem_seq(delta_max_index)
