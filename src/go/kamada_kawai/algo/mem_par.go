...
func (kk *KamadaKawai) Mem_par() {
	var delta_max_index int64 = kk.get_deltas_mem_par()
	...
	for {
		...
		for {
			...
			d_x_m, d_y_m, d_xx_m, d_yy_m, d_xy_m := kk.get_derivatives_mem_par(delta_max_index)
			...
			kk.Deltas[delta_max_index] = kk.get_delta_m_mem_par(delta_max_index)
		...
		var temp int64 = kk.update_deltas_mem_par(delta_max_index)
