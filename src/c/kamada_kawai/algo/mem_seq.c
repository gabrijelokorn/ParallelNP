...
void mem_seq(KamadaKawai *kk)
{
    int delta_max_index = get_deltas_mem_seq(kk);
    ...
    while (delta_max_index != -1)
    {
        ...
        while (kk->deltas[delta_max_index] > kk->epsilon)
        {
            ...
            get_derivatives_mem_seq(kk, delta_max_index, &d_x_m, &d_y_m, &d_xx_m, &d_yy_m, &d_xy_m);
        ...
        int temp = update_deltas_mem_seq(kk, delta_max_index);
