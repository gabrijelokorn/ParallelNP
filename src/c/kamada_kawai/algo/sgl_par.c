...
void sgl_par(KamadaKawai *kk)
{
    ...
    while (delta_max_index != -1)
    {
        ...
        while (kk->deltas[delta_max_index] > kk->epsilon)
        {
            ...
            get_derivatives_seq(kk, delta_max_index, &d_x_m, &d_y_m, &d_xx_m, &d_yy_m, &d_xy_m); // at first seeable speedup, then progressively worse overhead with more threads
            ...
            kk->deltas[delta_max_index] = get_delta_m_seq(kk, delta_max_index); // at first seeable speedup, then progressively worse overhead with more threads
            if (steps-- <= 0)
                break;
        }
