...
function mem_par(kk::KamadaKawai)
	delta_max_index = get_deltas_mem_par!(kk);

	while delta_max_index != -1
		steps = kk.limit

		while kk.deltas[delta_max_index] > kk.epsilon
			d_x_m, d_y_m, d_xx_m, d_yy_m, d_xy_m = get_derivatives_mem_par!(kk, delta_max_index)
		...
		temp = update_deltas_mem_par!(kk, delta_max_index);
